                    ...
                    <div class="card-body">
                        <div class="mb-3">
                            ...
                            <div class="input-group">
                                <input type="text" class="form-control" name="customer.mobile" id="custMobile" placeholder="Lookup Mobile..." required>
                                <button class="btn btn-outline-primary" type="button" onclick="fetchCustomer()"><i class="fa-solid fa-search"></i></button>
                            </div>
                        </div>
                        <div class="mb-3">
                            <label class="form-label text-muted-small">Customer Name</label>
                            <input type="text" class="form-control" name="customer.name" id="custName" placeholder="Enter Name">
                        </div>
                        <div class="mb-3">
                            <label class="form-label text-muted-small">Email (Optional)</label>
                            <input type="email" class="form-control" name="customer.email" id="custEmail" placeholder="email@example.com">
                        </div>
            ...
            <div class="col-md-8">
                <div class="card invoice-card">
                    <div class="card-header bg-white d-flex justify-content-between align-items-center">
                        ...
                        <button type="button" class="btn btn-sm btn-outline-success" onclick="addRow()"><i class="fa-solid fa-plus"></i> Add Item</button>
                    </div>
                    <div class="card-body">
                        <table class="table align-middle" id="itemsTable">
                            ...
                            <tr class="item-row">
                                <td>
                                    <select class="form-select" name="items[0].product.id" onchange="updateRow(this)">
                                        <option value="">Select Product</option>
                                        <c:forEach items="${products}" var="p">
                                            <option value="${p.productId}" data-price="500">${p.productName}</option>
                                        </c:forEach>
                                    </select>
                                </td>
                    ...
                    <div class="card-footer bg-light p-4">
                        <div class="row">
                            <div class="col-md-7">
                                ...
                                <textarea class="form-control" rows="3" placeholder="Notes for Monarch ERP records..."></textarea>
                            </div>
                            <div class="col-md-5">
...
<script>
    // 1. AJAX: Fetch Customer by Mobile
        ...
        fetch('/api/customers/search?mobile=' + mobile)
            ...
            .then(data => {
                if (data) {
                    ...
                    statusDiv.innerHTML = '<span class="text-success"><i class="fa-solid fa-check-circle"></i> Existing Customer Loaded</span>';
                } else {
                    statusDiv.innerHTML = '<span class="text-primary"><i class="fa-solid fa-info-circle"></i> New Customer - Please enter details</span>';
                }
            })
            .catch(err => {
                statusDiv.innerHTML = '<span class="text-muted">New entry mode active.</span>';
            });
    }
    ...
    function addRow() {
        ...
        newRow.innerHTML = `
            <td>
                <select class="form-select" name="items[\${rowCount}].product.id" onchange="updateRow(this)">
                    <option value="">Select Product</option>
                    <c:forEach items="${products}" var="p">
                        <option value="${p.productId}" data-price="500">${p.productName}</option>
                    </c:forEach>
